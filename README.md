...
> **教育現場16年で培った「校内SE」としての上流設計・実装力・調整力を、プロダクト開発に活かすエンジニア。**
> 公立学校で教育DXを主導しながら、現場の「負」を **動くプロダクト** に変えてきました。

250名規模の組織のITインフラ運用から、Excel VBA／GASによるRPA内製化（年間約200時間の余力創出）、Python（MeCab）による形態素解析でのデータ利活用までを一気通貫で完遂。並行してJavaScript／TypeScript／Pythonによる自社開発レベルのWebプロダクトを継続的に制作しています。教育の現場で「相手の理解」を起点に設計してきた経験を、そのままソフトウェアの体験設計に持ち込むことが強みです。

| | |
| :-- | :-- |
| 🎯 **志望** | 自社開発企業のフロントエンド〜フルスタックエンジニア／PM・PL候補 |
| 🧭 **得意領域** | 上流設計（要件定義・グランドデザイン）／フルスタック実装／ステークホルダーマネジメント |
| ♿ **大切にしていること** | アクセシビリティ・ユニバーサルデザイン・セキュリティ・属人化排除のドキュメント文化 |
| 📄 **詳細** | [職務経歴書（RESUME.md）](./RESUME.md) |

### 業務実績ハイライト

- **250名規模の組織**でITインフラのグランドデザイン〜運用までを一気通貫で完遂
- Excel VBA／GASによるRPA内製で **年間約200時間（約40%）の業務効率改善** を実現
- **端末100台規模の更新プロジェクト** を要件定義〜ベンダー選定〜定着支援まで主導
- Python（MeCab）による形態素解析で、アンケート等の定性データを定量化・可視化
- Active Directoryによるアカウント一元管理とセキュリティポリシー策定でITガバナンスを向上
...
| インフラ / 運用 | Vercel (CI/CD) / GitHub / Active Directory / Windows Server |
...
- Supabaseの15テーブルに **Row Level Securityを全面適用**、PINは **bcryptでハッシュ化**
- 子供向けサービスゆえの **COPPA／GDPRを意識したプライバシー設計**
- 約96コミットでv0.1→v0.22まで反復開発。要件定義・ER図・UX監査・マイグレーション設計をドキュメント化
...
- **簿記・会計領域** — 自ら **会計ソフトを開発する** 中で必要になった実務知識を、そのまま学習ツールへ。「作りながら学ぶ」を地で行く制作。
- **情報処理領域** — **自己デザイン**（学習者が自ら主体的に学ぶ）の思想に基づき、答えを与えるのではなく、試す・トレースする・自ら気づくプロセスを設計。
...
> 詳細な業務内容・実績は [職務経歴書（RESUME.md）](./RESUME.md) を参照ください。
