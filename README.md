...
**主な機能（予定含む）**
- 仕訳→試算表→精算表→決算書までの流れを自動採点
- 決算整理仕訳、減価償却の計算プロセスを個別に確認できる設計
- 財務分析12指標、CVP分析・損益分岐点、株式指標などの経営分析系メニュー
- 反復学習用のドリル機能と、学習進捗を可視化するダッシュボード

**開発状況**
現在はデスクトップアプリ版とWebアプリ版の両方を並行して検討中。教育現場で複数年使われてきた教材が土台にあるため機能要件はほぼ固まっており、実装フェーズに注力している段階です。
...
| 期間 | 所属 | 役割 |
| :-- | :-- | :-- |
| 2026.04〜現在 | 民間企業 | システム開発エンジニア（AI駆動開発） |
